...
class Word2Vec(object):
    ...
    def get_embedding(self, word):
        ...
        # TODO: embedding or normed embedding?
...
class Word2VecReader(object):
    ...
    def __init__(self, src_dir=None, vocab_size=50000):
        if src_dir is None:
            # TODO: adjust path after upload
            self.dir_path = '/Users/sblank/PycharmProjects/patents-public-data/models/landscaping/models'
        else:
            self.dir_path = os.path.join(src_dir, '/models/landscaping/models')
            if not os.path.exists(self.dir_path):
                raise Exception('Datapath does not exist:\n "{}"'.format(self.dir_path))
        self.model_name = '5.9m'
        self.vocab_size = vocab_size
        
        self.checkpoints_path = os.path.join(self.dir_path, self.model_name, 'checkpoints')
        self.checkpoints_file = os.path.join(self.checkpoints_path, '{}.ckpt'.format(self.model_name))
        self.vocab_dir = os.path.join(self.dir_path, self.model_name, 'vocab')
        self.vocab_file = os.path.join(self.vocab_dir, 'vocab.csv')
        self.config_file = os.path.join(self.vocab_dir, 'config.csv')
        self.train_words_path = os.path.join(self.dir_path, self.model_name, 'train_words.pkl')
